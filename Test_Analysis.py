...
if use_pickle == False:
    ...
    pivoted_sampled_datas_file = 'pivoted_sampled_datas.pickle'
    f = open(pivoted_sampled_datas_file, 'wb')
    pickle.dump(pivoted_sampled_datas, f)
    f.close()
else:
    pivoted_sampled_datas_file = 'pivoted_sampled_datas.pickle'
    f = open(pivoted_sampled_datas_file, 'rb')
    pivoted_sampled_datas = pickle.load(f)
    f.close()
...
for window_size in range(window_size_from, window_size_to):
    ...
    if use_window_size_pickle == False:
        ...
        pivoted_sampled_datas_zscore_window_size_file = 'pivoted_sampled_datas_zscore_window_size_%s.pickle' % (window_size)
        f = open(pivoted_sampled_datas_zscore_window_size_file, 'wb')
        pickle.dump(pivoted_sampled_datas_zscore, f)
        f.close()
    else:
        pivoted_sampled_datas_zscore_window_size_file = 'pivoted_sampled_datas_zscore_window_size_%s.pickle' % (window_size)
        f = open(pivoted_sampled_datas_zscore_window_size_file, 'rb')
        pivoted_sampled_datas_zscore = pickle.load(f)
        f.close()
            ...
            for column_nm in pivoted_sampled_datas_zscore.columns:
                ...
                if column_nm != index_nm:
                    ...
                    if model_cumulated_profit[index_nm][column_nm] > bm_cumulated_profit[index_nm][column_nm]:
                        print (window_size, '\t', index_nm, '\t', column_nm, '\t', model_cumulated_profit[index_nm][column_nm], '\t', bm_cumulated_profit[index_nm][column_nm], '\t', model_cumulated_profit[index_nm][column_nm]/bm_cumulated_profit[index_nm][column_nm]-1)


    # 데이터 저장
    if save_excel == True:
        save_excel_file_nm = 'test.xlsx'
        # 만약 해당 파일이 존재하지 않는 경우 생성
        workbook = xlsxwriter.Workbook(save_excel_file_nm)
        workbook.close()

        # Create a Pandas Excel writer using Openpyxl as the engine.
        writer = pd.ExcelWriter(save_excel_file_nm, engine='openpyxl')
        # 주의: 파일이 암호화 걸리면 workbook load시 에러 발생
        writer.book = openpyxl.load_workbook(save_excel_file_nm)
